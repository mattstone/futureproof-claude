#!/usr/bin/env python3
"""FutureProof — Platform Strategy & Build Brief (internal, full technical spec).

Renders the PDF *from* PLATFORM_BUILD_BRIEF.md so the markdown stays the single
source of truth: edit the .md, re-run this, the PDF follows. Supports headings,
paragraphs, bullet/numbered lists, pipe tables, fenced code/diagram blocks and
blockquote callouts, in the house navy/teal A4 style.

Body parsing starts after the `<!-- pdf:body-start -->` sentinel (the cover is
rendered from the constants below). A level-1 heading (`# `) starts a new page.
"""
import os
import re
...
SRC = 'PLATFORM_BUILD_BRIEF.md'
OUT = os.path.join('docs', 'pdfs', 'FutureProof_Platform_Strategy_and_Build_Brief_Jun2026.pdf')
USABLE = 170 * mm
...
def parse_body(s, lines):
    ...
    while i < len(lines):
        ...
        if line.strip().startswith('```'):                       # fenced code/diagram
            flush()
            i += 1
            buf = []
        ...
        if line.strip().startswith('<!--'):                      # sentinel/comment
            i += 1; continue

        if line.strip() == '---':                                # rule -> small gap
        ...
        if m:
            ...
            if level == 1:
                story.append(CondPageBreak(55 * mm))   # fresh page only if little room left
                story.append(Paragraph(inline(text), s['H1']))
            else:
                story.append(Paragraph(inline(text), s['H'+str(level)]))
            i += 1; continue

        if TABLE_RE.match(line):                                 # pipe table
            flush()
            rows = []
        ...
        if line.startswith('>'):                                 # callout
            ...
            joined = '<br/>'.join(inline(b) for b in buf)         # blank line -> double break
...
def cover(s):
    ...
    st.append(Paragraph('Platform Strategy &amp; Build Brief', s['Sub'])); st.append(Spacer(1, 2 * mm))
    st.append(Paragraph('Full technical build specification', s['SubL'])); st.append(Spacer(1, 16 * mm))
    st.append(Paragraph(
        'The build specification for the platform that turns one EPM product into one engine many licensed '
        'lenders run on, across markets. Architecture, domain model, four workstreams in build-level detail, '
        'the customer journeys, the build/test/deploy/operate process, security and compliance, and a phased '
        'roadmap with acceptance criteria &mdash; written so a developer (with AI coding agents) can start cold.',
        s['Call']))
    st.append(Spacer(1, 2 * mm))
    st.append(Paragraph('One engine. Every market. Run by agents.', s['SubL']))
...
def footer(canvas, d):
    ...
    canvas.drawString(20 * mm, 11 * mm,
                      'FutureProof  |  Platform Strategy & Build Brief  |  Internal  |  June 2026')
