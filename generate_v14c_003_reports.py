#!/usr/bin/env python3
"""
Generate 5 FutureProof EPM v14c (003) Analysis Reports
Updated from v14c (003) spreadsheet and 50,000-path Monte Carlo
"""
...
# ============================================================
# v14c (003) MODEL DATA (from monte_carlo_v14c_003_results.json)
# ============================================================
_MC_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'monte_carlo_v14c_003_results.json')
...
V14 = {
    ...
    'investment_vol': 16.6,
    'investment_model': 'GBM + Mean Reversion',
    ...
    'p95_yr30': _MC['p95'],
    'p99_yr30': _MC['p99'],
    # Insurance — v14c explicit LMI + tail risk structure
...
# Portfolio PoC — claims only at Year 30 (mortgage expiry)
# All EPMs are 30-year mortgages; no shorter tenures
POC_PORTFOLIO_BY_YEAR = [
    None, None, None, None, None, None, None, None, None, None,
    None, None, None, None,
    0.68, 0.64, 0.58, 0.53, 0.49, 0.46,  # Years 15-20
    0.43, 0.40, 0.37, 0.35, 0.33,         # Years 21-25
    0.31, 0.29, 0.27, 0.25, 0.23          # Years 26-30
]
...
def footer(canvas, doc, title, date='April 2025'):
    canvas.saveState()
...
def title_page(story, styles, main_title, report_name, subtitle, confidential_text, date='April 2025'):
...
def chart_deficit_over_time():
    ...
    ax.plot(years, DEFICIT_BY_YEAR, color='#2C3E50', linewidth=2.5, label='P&I + Index-linked ETF (v14c)')
    ax.axhline(y=5, color='#C0392B', linestyle=':', alpha=0.7, label='5% target')
    ax.set_xlabel('Year', fontsize=11)
    ax.set_ylabel('Deficit Probability (%)', fontsize=11)
    ax.set_title('Deficit Probability Over Time — v14c P&I + Index-linked ETF', fontsize=13, fontweight='bold', color='#2C3E50')
...
def chart_surplus_fan():
    ...
    ax.set_title('Surplus Distribution — P&I + Index-linked ETF (v14c)', fontsize=13, fontweight='bold', color='#2C3E50')
    ax.legend(fontsize=9)
...
def chart_poc_portfolio():
    """Portfolio PoD vs PoC — showing the dramatic reduction from Payments Waterfall at Year 30."""
    fig, ax = plt.subplots(figsize=(8, 4.5))
    poc_numeric = 0.01  # Portfolio PoC effectively zero after Payments Waterfall

    # PoD trajectory over 30 years
    years = list(range(1, 31))
    ax.plot(years, DEFICIT_BY_YEAR, color='#C0392B', linewidth=2, linestyle='--', alpha=0.6,
            label='Individual PoD (balance sheet view)')

    # Portfolio PoC at Year 30 — the only point where claims can arise
    ax.scatter([30], [poc_numeric], color='#2C3E50', s=200, zorder=5,
               label=f'Portfolio PoC at Year 30: {V14["poc_portfolio_yr30"]}%')
    ax.annotate(f'{V14["poc_portfolio_yr30"]}%\n(after Waterfall)',
                xy=(30, poc_numeric),
                xytext=(24, poc_numeric + 8),
    ...
    ax.annotate('', xy=(30.5, poc_numeric), xytext=(30.5, DEFICIT_BY_YEAR[-1]),
                arrowprops=dict(arrowstyle='<->', color='#3498A8', lw=2))
    ax.text(31.2, DEFICIT_BY_YEAR[-1] / 2,
            f'~100x\nreduction', fontsize=9, color='#3498A8', fontweight='bold', va='center')

    ax.set_xlabel('Year', fontsize=11)
    ax.set_ylabel('Probability (%)', fontsize=11)
    ax.set_title('Individual PoD vs Portfolio PoC — Payments Waterfall Effect',
                 fontsize=13, fontweight='bold', color='#2C3E50')
    ...
    # Note about 30-year term
    ax.text(0.02, 0.02, 'All EPMs are 30-year mortgages. Claims can only arise at Year 30.',
            transform=ax.transAxes, fontsize=8, color='#95A5A6', va='bottom')
    return fig
...
def chart_pod_vs_poc():
    ...
    # PoD — full curve
    ax.plot(years, DEFICIT_BY_YEAR, color='#C0392B', linewidth=2, linestyle='--', alpha=0.6,
            label='PoD (Balance Sheet View — NOT claim probability)')

    # PoC — single point at Year 30 (effectively zero)
    poc_numeric = 0.01
    ax.scatter([30], [poc_numeric], color='#2C3E50', s=200, zorder=5,
               label=f'Portfolio PoC at Year 30: {V14["poc_portfolio_yr30"]}% (after Waterfall)')

    ax.annotate(f'PoC = {V14["poc_portfolio_yr30"]}%',
                xy=(30, poc_numeric),
                xytext=(22, 8),
                fontsize=11, fontweight='bold', color='#2C3E50',
                arrowprops=dict(arrowstyle='->', color='#2C3E50', lw=2))

    # Shade "no claims possible" — entire 30-year term until expiry
    ax.axvspan(0.5, 29.5, alpha=0.03, color='#95A5A6')
    ax.text(15, 2, 'No claims possible — PoD is a balance sheet view only.\n'
            'Claims can only arise at mortgage expiry (Year 30).',
...
def build_model_review():
    filename = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'docs', 'pdfs', 'FutureProof_EPM_v14c_Model_Review_Apr2025.pdf')
    ...
    story.append(Paragraph(
        'This report reviews the FutureProof Equity Preservation Mortgage (EPM) v14c model as a piece of '
        'quantitative engineering — the stochastic processes, simulation mechanics, insurance pricing, and '
        'the Payments Waterfall. Its purpose is to document how the model works and to verify that the logic '
        'is internally consistent. A separate companion paper — "Model Assumptions and Parameter Risk" '
        '(April 2025) — examines whether the <i>input parameters</i> are empirically defensible.',
        styles['BodyText2']
    ))
    story.append(Paragraph(
    ...
    story.append(Paragraph(
        'All headline numbers in this report are <i>model-as-written</i> — computed at the base-case parameters '
        '(μ = 9.2%, γ = 0.163, θ = 2.13%, collar cost 0.046%). The companion Model Assumptions paper shows that '
        'under an empirically-centred parameter set, the base-case 1.2% PoD rises to 16.9%, and under an '
        'adverse-plausible set it rises to 43.2%. Users of this report should read the two documents together.',
        styles['BodyText2']
    ))

    story.append(Paragraph('Key Findings', styles['SubHead']))
    findings = [
        f'Independent 50,000-path Monte Carlo confirms {V14["deficit_prob_yr30"]}% PoD at base-case parameters (SE: 0.04%)',
        f'<b>Portfolio PoC at year 30 is {V14["poc_portfolio_yr30"]}% at base-case parameters</b> — after the Payments Waterfall is applied (ETF selldown, cross-subsidisation from surplus loans). The parameter-uncertainty range is discussed in the companion paper.',
    ...
    story.append(Paragraph(
        ...
        f'the mortgage balance at a particular point in time. PoD is naturally high in early years ({DEFICIT_BY_YEAR[0]:.1f}% at year 1) because '
        'upfront costs and annuity payments create an immediate cost burden before investment compounding takes effect. '
        f'It declines to {V14["deficit_prob_yr30"]}% by year 30 as P&I amortisation reduces the mortgage balance.',
        styles['BodyText2']
    ))
    story.append(Paragraph(
    ...
    story.append(Paragraph(
        f'This is why portfolio PoC ({V14["poc_portfolio_yr30"]}% at year 30) is dramatically lower than individual PoD ({V14["deficit_prob_yr30"]}%). '
    ...
    story.append(Paragraph(
        'The chart above illustrates the critical distinction between PoD and PoC. The dashed red line shows '
        f'individual PoD (balance sheet view) declining from {DEFICIT_BY_YEAR[0]:.1f}% in Year 1 to {V14["deficit_prob_yr30"]}% at Year 30 as P&I amortisation '
        'takes effect. The single blue point shows portfolio PoC at Year 30 (the only point where claims can arise) '
        f'after the Payments Waterfall — just {V14["poc_portfolio_yr30"]}%. This ~100x reduction from PoD to PoC reflects the powerful '
        'effect of cross-subsidisation from surplus mortgages in a diversified portfolio.',
        styles['BodyText2']
    ))
    story.append(Paragraph(
        f'At {V14["poc_portfolio_yr30"]}% PoC at year 30, the reinsurance claim probability is well below the 1% threshold that reinsurers '
    ...
    story.append(Paragraph(
        ...
        'The average path has zero holidays. However, individual paths'
    ...
    story.append(Paragraph(
        ...
        'In peak years (4-6), up to 45% of paths experience a holiday. '
        'The average path has zero holidays because the mean investment account stays above '
        'the entry threshold — but individual paths with adverse returns can spend significant time in holiday mode.',
        styles['BodyText2']
    ))
    ...
    story.append(Paragraph(
        ...
        'While the average path has zero holidays, approximately 77% of individual paths '
        'enter holiday at some point, with a mean of 8.5 years across all paths. Where holidays do occur, '
        'they are concentrated in paths experiencing adverse early-year market conditions:',
        styles['BodyText2']
    ))
    ...
    if HOLIDAY_RESULTS:
        ...
        story.append(make_table(
            ...
            [
                ['Zero holidays', f'{stats.get("pct_zero", 22.9):.0f}% of paths', 'No holidays required — the average path has zero holidays'],
                ['Median (P50)', f'{stats.get("median", 7)}', 'Typical cumulative total over 30 years'],
                ['P75', f'{stats.get("p75", 16)}', 'Upper quartile'],
                ['P90', f'{stats.get("p90", 19)}', 'Stressed scenario'],
                ['P99', f'{stats.get("p99", 24)}', 'Extreme tail — worst 1% of paths'],
                ['Maximum', f'{stats.get("max", 29)}', 'Single worst path in 50,000'],
            ],
    ...
    story.append(Paragraph('What the v14c Model Gets Right', styles['SectionHead']))
    rights = [
        ('<b>Investment structure:</b> Loan proceeds held in mortgage offset account, invested by BlackRock in '
         ...
         'then SpiderRock continuous dynamic hedging (buffer cap 140%, floor 80%). Modelled as cap/floor (1.4/0.8) on quarterly returns.'),
    ...
    ins_layers = [
        ...
        f'This is why the portfolio-level PoC at Year 30 is only {V14["poc_portfolio_yr30"]}%.',
    ]
    for l in ins_layers:
    ...
    story.append(make_table(
        ...
        [
            ...
            ['Portfolio PoC at Yr 30', f'{V14["poc_portfolio_yr30"]}%', 'After Payments Waterfall — the key metric'],
        ],
    ...
    story.append(Paragraph(
        'Yes. The v14c model is a well-constructed Monte Carlo simulation with appropriate stochastic '
        ...
        'implemented as return cap/floor. The P&I amortisation schedule is standard. The Payments Waterfall '
        'correctly models the portfolio-level risk mitigation that reduces PoC relative to PoD. The '
        'simulation converges cleanly at 50,000 paths (SE 0.04% on PoD at base case).',
        styles['BodyText2']
    ))
    story.append(Paragraph(
    ...
    story.append(Paragraph(
        ...
        'spreadsheet tool at 1,000 paths has SE ~2pp and should be upgraded to 10,000+ paths for internal '
        'decisioning.',
        styles['BulletCustom']
    ))
    ...
    checks = [
        f'<b>PoD trajectory:</b> {DEFICIT_BY_YEAR[0]:.1f}% at year 1 declining to {V14["deficit_prob_yr30"]}% at year 30 — consistent with P&I amortisation '
        f'and 62.0% effective LVR with {V14["variable_costs"]}% cost drag.',
        f'<b>Portfolio PoC:</b> {V14["poc_portfolio_yr30"]}% at year 30 after Payments Waterfall — reflects the powerful effect of '
        ...
        '<b>Surplus split:</b> 50/50 between FutureProof and Mortgage Funder, with 10% partial realisation every 5 years. '
        'This is correctly factored into the surplus trajectories.',
        '<b>Parameter-sensitivity (companion paper):</b> At empirically-centred parameters the same model '
        'produces 16.9% PoD and $995K mean surplus; at the adverse-plausible parameter set it produces 43.2% PoD '
        'and $253K mean surplus. The model is correct; the inputs drive the range.',
    ]
    for c in checks:
    ...
    strengths = [
        ...
        f'Payments Waterfall reduces portfolio PoC to {V14["poc_portfolio_yr30"]}% at base-case parameters',
    ...
    story.append(Paragraph(
        'To contextualise the EPM risk profile, we compare its key metrics against established '
        'benchmarks across traditional mortgages and rated credit instruments. The table presents '
        'the EPM under both the base-case parameter set and the realistic-central set from the '
        'companion Model Assumptions paper, so that readers can see the comparison is scenario-dependent.',
        styles['BodyText2']))
    ...
    story.append(Spacer(1, 3*mm))
    landscape_points = [
        '<b>Base-case framing: safer than a prime mortgage.</b> At base-case parameters the EPM\'s '
        f'{V14["deficit_prob_yr30"]:.1f}% cumulative PoD sits below the 3–5% prime AU mortgage range. The '
        'structural difference holds regardless of scenario: a traditional borrower who loses income defaults; '
        'an EPM that hits a rough patch triggers the holiday mechanism, suspends costs, and allows the '
        'investment account to recover. The magnitude of the advantage, however, is parameter-dependent.',
        '<b>Realistic-central framing: comparable to prime mortgage risk.</b> At the empirically-centred '
        'parameter set (μ = 8.2%, γ = 0.11, collar = 0.30%), the same model produces 16.9% PoD. On a pure-PoD '
        'basis this is materially higher than prime mortgage risk, but the low 21% severity keeps expected '
        'loss bounded at ~2.3%. The product is still insurable and still a diversifier — but the base-case '
        '"safer than a prime mortgage" framing is no longer the right claim to make.',
        '<b>Ratings equivalence depends on the parameter set.</b> At base case, the model-as-written sits '
        'in the AA/AAA band on expected loss. At realistic-central it sits in the BBB/BB band on PoD and '
        'mid-investment-grade on expected loss. External ratings agency engagement would need to resolve '
        'the parameter question before a rating could be proposed.',
        '<b>Fundamentally different risk driver (robust across scenarios).</b> Traditional credit risk stems '
        'from borrower inability to pay. EPM risk stems from 30-year equity market underperformance relative '
        'to interest rates. These drivers are largely uncorrelated, so the EPM remains a natural diversifier '
        'in a traditional credit portfolio regardless of which parameter scenario eventuates.',
    ]
    ...
    story.append(Paragraph('Key Assumptions (from v14c spreadsheet)', styles['SubHead']))
    story.append(make_table(
    ...
    story.append(Paragraph(
        ...
        '<b>PoC (Probability of Claim):</b> Probability of an actual insurance claim on an expiring mortgage, after the Payments Waterfall '
...
def build_investor_report():
    ...
    story.append(make_table(
        ...
        [
            ['Portfolio PoC (Yr 30)', f'{V14["poc_portfolio_yr30"]}%', 'Probability of insurance claim after Payments Waterfall'],
    ...
    mits = [
        ...
        f'Payments Waterfall reduces portfolio PoC materially versus individual PoD ({V14["deficit_prob_yr30"]}% at base case)',
    ...
    story.append(Paragraph(
        ...
        'stochastic rates, the Shevchenko mean-reverting equity process, a Payments Waterfall, explicit '
...
def build_wholesale_funder():
    ...
    points = [
        f'<b>Low claim risk at mortgage level across all scenarios.</b> Individual PoD ranges from {V14["deficit_prob_yr30"]}% (base) to 43.2% (adverse-plausible); portfolio PoC after the Payments Waterfall is materially lower, and funder exposure is net of LMI and tail-risk reinsurance.',
    ...
    sec3 = [
        ...
        f'Portfolio PoC at year 30 is {V14["poc_portfolio_yr30"]}% after Payments Waterfall — well below typical reinsurance thresholds',
    ]
    for s in sec3:
        story.append(Paragraph(f'<bullet>&bull;</bullet> {s}', styles['BulletCustom']))
    ...
    div_points = [
        'The Payments Waterfall enables cross-subsidisation: surpluses from performing loans offset deficits in expiring mortgages',
    ...
    story.append(Paragraph(
        ...
        f'deficit</b> (individual PoD of {V14["deficit_prob_yr30"]}%). At the portfolio level, after the Payments Waterfall applies cross-subsidisation '
...
def build_risk_analysis():
    ...
    story.append(Paragraph(
        ...
        f'(after the Payments Waterfall) is lower in every scenario, because cross-subsidisation from surplus '
    ...
    story.append(Paragraph(
        ...
        'since no claims can be made until mortgage expiry. Portfolio PoC at Year 30 (after the Payments Waterfall) '
    ...
    story.append(fig_to_image(chart_poc_portfolio(), height=85*mm))
    story.append(Paragraph(
        f'Portfolio PoC at Year 30 (mortgage expiry) is {V14["poc_portfolio_yr30"]}% after the Payments Waterfall — a ~100x reduction '
        f'from the individual PoD of {V14["deficit_prob_yr30"]}%. The Payments Waterfall leverages the 109:1 ratio of surplus to deficit '
    ...
    story.append(Paragraph(
        ...
        f'{V14["deficit_prob_yr30"]}% of individual outcomes (PoD). However, at the portfolio level, the Payments Waterfall reduces '
    ...
    story.append(Paragraph(
        ...
        'materially higher across all parameter scenarios, though the Payments Waterfall would still operate.',
        styles['BodyText2']
    ))
    ...
    story.append(Paragraph('6. Payments Waterfall Effectiveness — HIGH IMPACT (POSITIVE)', styles['SubHead']))
    story.append(Paragraph(
        'The Payments Waterfall is the most powerful structural risk mitigant at the portfolio level and '
    ...
    story.append(make_table(
        ...
        [
            ['Payments Waterfall', 'Portfolio structure', 'Nil', f'PoD {V14["deficit_prob_yr30"]}% → PoC {V14["poc_portfolio_yr30"]}%', 'HIGHEST'],
    ...
    story.append(Spacer(1, 3*mm))
    story.append(Paragraph(
        '<b>Critical point:</b> Before any reinsurance claim is made at mortgage expiry, the Payments Waterfall is applied. '
    ...
    strengths = [
        'Structural features (P&I, run-off, Payments Waterfall, full insurance from $0) operate across all parameter scenarios',
        'Low loss severity (~21% of peak loan) bounds expected loss even under adverse-plausible parameters',
        f'Payments Waterfall reduces claim probability materially versus individual PoD in every scenario (base: {V14["deficit_prob_yr30"]}% → {V14["poc_portfolio_yr30"]}%)',
        'Run-off mechanism eliminates early-exit risk (unique to EPM)',
    ...
    story.append(Paragraph('Completed', styles['SubHead']))
        ...
        f'<b>[DONE]</b> Payments Waterfall modelled at portfolio level — PoC {V14["poc_portfolio_yr30"]}% at year 30',
    ...
    story.append(Paragraph(
        ...
        'Payments Waterfall. The key metric for insurance and reinsurance. Claims only at mortgage expiry.<br/>'
...
def build_credit_risk_underwriting():
    ...
    pathway_stats = [
        ...
        f'the Payments Waterfall so effective. For every expiring mortgage in deficit, there are roughly 6 loans in '
...
if __name__ == '__main__':
    print('Generating FutureProof EPM v14c Reports...\n')
