#!/usr/bin/env python3
"""
50,000-path Monte Carlo simulation of the FutureProof EPM v14 model.
Faithfully replicates the VBA spreadsheet mechanics:
  - ANNUAL time steps (not quarterly)
  - Investment account: grows by equity returns, shrinks by interest + margins + hedge fee
  - Annuity ADDS to loan balance (not deducted from investment)
  - No explicit P&I amortization payments
  - Holiday based on InvestmentAccount vs InitialLoan * threshold
  - Ornstein-Uhlenbeck cash rate model (not CIR)
  - Annual lognormal returns with buffer cap/floor
"""
...
# ============================================================
# v14 PARAMETERS (from spreadsheet)
# ============================================================
...
EQUITY_MEAN = 0.10       # mu for lognormal: Exp(mu + vol*Z) - 1
EQUITY_VOL = 0.10        # vol for lognormal
BUFFER_CAP = 1.20        # cap on (1+return), i.e. max return +20%
BUFFER_FLOOR = 0.80      # floor on (1+return), i.e. min return -20%

# Cash rate (Ornstein-Uhlenbeck)
CASH_RATE_INITIAL = 0.044
CASH_RATE_THETA = 0.044   # Long-run mean
CASH_RATE_KAPPA = 0.80    # Mean-reversion speed
CASH_RATE_SIGMA = 0.015   # Volatility
CASH_RATE_EQUITY_CORR = 0.002

# Holiday mechanism
HOLIDAY_ENTRY_LEVEL = 0.9    # threshold as fraction of InitialLoan
HOLIDAY_EXIT_LEVEL = 1.458   # threshold as fraction of InitialLoan
...
def run_simulation():
    ...
    print(f"Parameters: Loan=${INITIAL_LOAN:,}, Annual steps, Buffer ETF, Stochastic OU rates")
    start = time.time()
    ...
    collar_price = -0.004  # net collar earns ~0.4% of investment
    ...
    # ============================================================
    print("\n" + "="*70)
    print("RESULTS — 50,000 PATH MONTE CARLO SIMULATION (v14 PARAMETERS)")
    print("="*70)
    ...
    with open('monte_carlo_v14_results.json', 'w') as f:
        json.dump(results, f, indent=2)
    print(f"\nResults saved to monte_carlo_v14_results.json")
