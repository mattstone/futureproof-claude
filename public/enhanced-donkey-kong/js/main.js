...
function start() {
  ...
  function downFloor() {
    ...
    const pMario = mario.getBoundingClientRect();
    const peachElement = document.querySelector(".peach");
    
    // Check if peach element exists before getting its rect
    if (!peachElement) return;
    
    const pPeach = peachElement.getBoundingClientRect();
