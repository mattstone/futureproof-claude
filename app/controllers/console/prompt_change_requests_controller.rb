...
class Console::PromptChangeRequestsController < Console::BaseController
  def index
    @records = PromptChangeRequest.includes(:user).recent_first.page(params[:page]).per(25)
  end
  ...
  def refresh
    ...
    redirect_to console_prompt_change_request_path(@request), notice: "Status refreshed."
  rescue GithubBridge::BridgeError => e
    redirect_to console_prompt_change_request_path(@request), alert: e.message
  end
  ...
  def submit_to_github(request)
    ...
    redirect_to console_prompt_change_request_path(request),
