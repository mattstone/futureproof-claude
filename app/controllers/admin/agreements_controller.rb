  ...
  class AgreementsController < BaseController
    ...
    def index
      ...
      unless current_jurisdiction == "Summary"
        @agreements = @agreements.where(jurisdiction: current_jurisdiction)
      end
      ...
      if params[:q].present?
        @agreements = @agreements.where("title ILIKE ?", "%#{params[:q]}%")
      end
    ...
    def create
      ...
      if @agreement.save
        redirect_to admin_agreement_path(@agreement), notice: "Agreement created. You can now edit and customise it."
      else
    ...
    def edit
      unless @agreement.editable?
        redirect_to admin_agreement_path(@agreement), alert: "Only draft agreements can be edited."
      end
    end
    ...
    def update
      ...
      if @agreement.update(agreement_params)
        redirect_to admin_agreement_path(@agreement), notice: "Agreement updated."
      else
    ...
    def send_for_signing
      ...
      redirect_to admin_agreement_path(@agreement), notice: "Agreement sent for signing."
    rescue => e
      redirect_to admin_agreement_path(@agreement), alert: e.message
    end

    def sign
      @signer_role = params[:role] || "counterparty"
      if @agreement.signed_by?(@signer_role)
        redirect_to admin_agreement_path(@agreement), alert: "#{@signer_role.titleize} has already signed."
      end
    end
    ...
    def record_signature
      ...
      if @agreement.status_fully_executed?
        redirect_to admin_agreement_path(@agreement), notice: "Agreement fully executed! Both parties have signed."
      else
        redirect_to admin_agreement_path(@agreement), notice: "Signature recorded successfully."
      end
    rescue => e
      redirect_to sign_admin_agreement_path(@agreement, role: params.dig(:signature, :signer_role)), alert: e.message
    end

    def cancel
      @agreement.cancel!
      redirect_to admin_agreements_path, notice: "Agreement cancelled."
    rescue => e
      redirect_to admin_agreement_path(@agreement), alert: e.message
    end
    ...
    def load_parties(party_type)
      case party_type
      when "Lender" then Lender.order(:name)
