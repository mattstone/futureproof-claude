...
def single_mortgage(total_loan, reinvest_fraction, loan_duration, annual_income, annuity_duration,
  ...
  is_cash_rate_list = type(cash_rate_series) is list
  ...
  annuity_duration_quarters = annuity_duration * 4
  ...
  for (pathn,S) in price_paths:
    ...
    data.append([pathn, 0, year_offset, 0, S0, 0, round(total_loan * reinvest_fraction), holdings, round(holdings*S0),
                round(0), round(max(loan_size - holdings*S0 ,0)), round(holdings*S0 - loan_size - deferred), in_holiday, 0,
                  annual_income_quarter, 0, False, 0, 0, 0, 0, 0,0,0,0,0,init_units_to_principal,0,0 ])

    last_yearly_hedge_price = S0
    last_5yearly_hedge_price = S0
    
    # Pre-calculate array indices for better cache performance
    total_periods = 4 * loan_duration + 1
    
    for t in range(1, total_periods):
      # Pre-calculate index once
      price_index = int(t * dt_quarter_inv) - 1
      s = S[price_index] # current price

      # Pre-calculate cash rate
      cash_rate = cash_rate_series[price_index] if is_cash_rate_list else cash_rate_series
      ...
      # Pre-calculate common values (but keep calculations in exact same order as original)
      ...
      # Keep original year/quarter calculation for accuracy
      year = math.floor((t-1)/4) +1
      quarter = t - 4*math.floor((t-1)/4)
      
      # Pre-calculate rounded values (use exact same calculations as original)
      loan_size_rounded = round(loan_size)
      holdings_value_rounded = round(holdings*s)
      capital_deficit_rounded = round(max(loan_size - holdings*s, 0))
      surplus_rounded = round(holdings*s - loan_size - deferred + cum_units_to_pool * s)
      
      data.append([pathn, t, year_offset+year, quarter, s, round(interest_due), loan_size_rounded, holdings, holdings_value_rounded,
                  deferred, capital_deficit_rounded, surplus_rounded, in_holiday, funder_earned,
