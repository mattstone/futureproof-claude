...
house_value = 2000000  # @param {type:"slider", min:500000, max:5000000, step:10000}
...
annual_income = house_value * 0.015
...
annual_house_price_appreciation = (
    0.04  # @param {type:"slider", min:0.01, max:0.08, step:0.0025}
)
...
wholesale_lending_margin = 0.03  # OPTIMIZED: 1% wholesale lending margin

# retail lending, FP loan margin. not premium
additional_loan_margins = 0.012  # OPTIMIZED: 0% additional FP margin

# interest_on_deferred = 0
### @param {type:"slider", min:0.00, max:0.09, step:0.0025}

# OPTIMIZED: Payment holidays disabled (impossible to enter)
holiday_enter_fraction = 0.9  # @param {type:"slider", min:0.5, max:3.5, step:0.05}
holiday_exit_fraction = 1.4  # @param {type:"slider", min:0.5, max:3.5, step:0.05}
...
max_superpay_factor = 1.5  # Default value

# insurance_cost_pa = 0.005  # OPTIMIZED: 0.5% per annum (down from 2%)
insurance_cost_pa = 0.25  # OPTIMIZED: 0.5% per annum (down from 2%)
...
hedged = True  # Default no hedging
# hedging_max_loss = 0.1  # Default 10% max loss
hedging_max_loss = 0.2  # Default 10% max loss
# hedging_cap = 0.2  # Default 20% cap
hedging_cap = 0.4  # Default 20% cap
# hedging_cost_pa = 0.01  # Default 1% cost per annum
hedging_cost_pa = 0.005  # Default 1% cost per annum
