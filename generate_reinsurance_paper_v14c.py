...
Based on v14c (003) model — GBM+MeanRev equity model (Shevchenko 2026)
50,000-path Monte Carlo simulation

De Ravin feedback incorporated:
  - Stochastic model selection section (GBM vs GBM+MeanRev, Shevchenko paper)
  - MLE parameter estimation methodology
  - Per-mortgage clarification throughout (not global constants)
  - 100% insurance from dollar zero, split by loss severity (not XoL with attachment)
  - Both premiums upfront at origination
  - Interest holiday (not annuity holiday)
  - LMI insurer is first-loss risk bearer (not homeowner)
  - 30-year mortgage only (no 10/15-year references)
  - Fixed deficit distribution chart
"""
...
def chart_insurance_structure():
    ...
    # Reconstruct approximate conditional deficit distribution from model data
    # 5.88% of 50,000 paths = ~2,940 deficit paths
    # Mean conditional deficit ~$463K, top cover boundary at P20 = $735K
    np.random.seed(42)
    n_deficit = int(MC['n_paths'] * MC['deficit_prob'] / 100)

    # Use a mix that matches the known statistics:
    # mean deficit ~$463K, P20 boundary ~$735K, range up to ~$2M+
    # Log-normal fit: if mean=463K and P80=735K => mu~12.8, sigma~0.65
    deficits = np.random.lognormal(mean=12.8, sigma=0.65, size=n_deficit)
    deficits = np.clip(deficits, 10000, 3_000_000)
    ...
    bins = np.linspace(0, 2_500_000, 50)
    ...
    ax.set_title('Per-Mortgage Conditional Deficit Distribution\nTwo-Layer Insurance Split (v14c, $2M home)',
...
def chart_poc_trajectory():
    ...
    ax.set_title('PoD Trajectory Over Mortgage Term (v14c, per-mortgage)', fontsize=13,
...
def chart_premium_allocation():
    ...
    ax.set_title('Per-Mortgage Fair Premium Allocation\nLMI vs Tail Risk Reinsurance ($2M home)',
                 fontsize=12, fontweight='bold', color='#2C3E50')
    return fig
...
def build_reinsurance_paper():
    ...
    filename = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                            'docs', 'pdfs', 'FutureProof_EPM_Reinsurance_Structure_Apr2025.pdf')
    ...
    story.append(Paragraph(
        'Based on the Equity Preservation Mortgage (EPM) v14c Model<br/>'
        'GBM + Mean Reversion Equity Model (Shevchenko 2026)<br/>'
    ...
    story.append(Paragraph(
        '<b>Important note on dollar amounts:</b> All dollar amounts in this paper are per-mortgage values for a '
        'representative $2,000,000 property at 80% LVR. These amounts will vary proportionally for different '
        'home values, LVR ratios, and annuity levels. They are not global constants.',
        styles['Callout']
    ))
    ...
    story.append(Paragraph('Key Findings (per-mortgage, $2M home at 80% LVR)', styles['SubHead']))
    ...
    story.append(Paragraph(
        ...
        'from historical data, 0.046% collar cost, base correlation). A companion paper — '
    ...
    story.append(Paragraph(
        ...
        '~21% loss severity given a deficit, 100% insurance from dollar zero, ~30-year float on upfront premium) '
        'are scenario-robust — they hold under every parameter set tested. What moves with the scenarios is '
        'expected loss frequency and, consequently, fair premium. Reinsurance pricing should anchor to the '
        'realistic-central scenario; capital and limit sizing should anchor to the adverse-plausible leg, '
        'not the base case.',
        styles['Callout']
    ))
    story.append(PageBreak())
    ...
    story.append(Paragraph(
        ...
        'equity market returns and interest rates. This section describes the model selection process, '
    ...
    story.append(make_table(
        ...
        [
            ...
            ['Loss severity (structural)', '~21% of deficit',
    ...
    story.append(Paragraph(
        '<b>What is scenario-robust:</b> the ~21% loss severity on deficit paths, the absence of prepayment '
        'risk (run-off mechanism), the cross-subsidy benefit at the portfolio level, the ~30-year float on '
        'upfront premium, and the zero-attachment-point structure of combined LMI + reinsurance. These are '
        'structural features of the product, not parameter-dependent.',
        styles['Callout']
    ))
    story.append(PageBreak())
    ...
    story.append(Paragraph(
        ...
        '(those with smaller deficits, up to the split boundary).',
        styles['FPBullet']
    ))
    story.append(Paragraph(
        '<bullet>&bull;</bullet> <b>Tail Risk Reinsurance:</b> On the remaining 20% of deficit paths '
        '(those with larger deficits), the LMI insurer pays up to the split boundary and the reinsurer '
        'pays the excess above it.',
        styles['FPBullet']
    ))
    story.append(Paragraph(
        '<bullet>&bull;</bullet> <b>Combined effect:</b> The wholesale mortgage funder bears zero credit risk — '
        'every deficit is fully covered from the first dollar.',
        styles['FPBullet']
    ))

    story.append(Paragraph('The Payments Waterfall', styles['SubHead']))
    story.append(Paragraph(
        'Before any insurance claim is triggered, a multi-step Payments Waterfall applies at the portfolio level. '
        'This waterfall dramatically reduces the actual claim frequency:',
        styles['BodyText2']
    ))
    story.append(Paragraph(
    ...
    story.append(Spacer(1, 3*mm))
    story.append(Paragraph(
        f'The Payments Waterfall reduces portfolio PoC from {MC["deficit_prob"]}% (individual mortgage) to '
        'effectively zero (portfolio of 10,000+ EPMs) — the 109:1 surplus-to-deficit ratio ensures '
        'near-complete cross-subsidisation.',
        styles['Callout']
    ))
    ...
    story.append(Paragraph(
        ...
        f'for each mortgage. For the representative $2M home shown above, this boundary is '
    ...
    story.append(Paragraph(
        ...
        'boundary, the reinsurer pays the excess above the boundary. The reinsurer has no liability on the '
        'other 80% of deficit paths.',
        styles['FPBullet']
    ))
    ...
    story.append(Paragraph(
        'The LMI program provides first-loss credit protection to the wholesale mortgage funder on every EPM '
        'originated. Unlike traditional LMI (which covers shortfalls from foreclosure sales), EPM LMI covers '
        'the deficit between the investment account value and the outstanding mortgage balance at mortgage expiry.',
        styles['BodyText2']
    ))

    story.append(Paragraph('4.2 LMI Program Settings (per-mortgage, $2M home)', styles['SubHead']))
    story.append(make_table(
        ...
        [
            ...
            ['Conditional expected deficit', f'${abs(INS["lmi"]["cond_expected_deficit"]):,.0f} per mortgage', 'Mean deficit given a deficit occurs'],
    ...
    story.append(Paragraph(
        ...
        f'${abs(INS["top_cover_limit"]):,.0f} (for a $2M home). Larger deficits are shared with the reinsurer.',
        styles['FPBullet']
    ))
    story.append(Paragraph(
    ...
    story.append(Paragraph(
        ...
        'periods of underperformance. When the investment account falls below 90% of the initial mortgage '
    ...
    story.append(Paragraph(
        '<bullet>&bull;</bullet> This mechanism reduces the probability of deficit by preserving the investment '
        'account during downturns. While the average path has zero holidays, '
        f'approximately {100 - MC["pct_zero_holidays"]:.0f}% of individual paths enter holiday at some point, '
        f'with a mean of {MC["mean_total_holiday_years"]:.1f} years across all paths.',
        styles['FPBullet']
    ))
    story.append(PageBreak())
    ...
    story.append(Paragraph('5.2 Reinsurance Program Settings (per-mortgage, $2M home)', styles['SubHead']))
    story.append(make_table(
        ['Setting', 'Value', 'Rationale'],
        [
    ...
    story.append(Paragraph(
        ...
        f'(up to ${abs(INS["top_cover_limit"]):,.0f} per mortgage for a $2M home).',
        styles['FPBullet']
    ))
    story.append(Paragraph(
    ...
    story.append(Spacer(1, 3*mm))
    story.append(Paragraph(
        f'<b>Worked example (per-mortgage, $2M home):</b> If a mortgage expires with a $900,000 deficit '
        f'(after the Payments Waterfall), the LMI insurer pays ${abs(INS["top_cover_limit"]):,.0f} and the '
        f'reinsurer pays ${900_000 - abs(INS["top_cover_limit"]):,.0f}. If the deficit is $500,000, the LMI '
        f'insurer pays the full $500,000 and the reinsurer pays nothing.',
        styles['Callout']
    ))
    ...
    story.append(Paragraph(
        ...
        'The reinsurance premium reflects only the excess-of-boundary component.',
        styles['BodyText2']
    ))
    story.append(Paragraph(
    ...
    story.append(make_table(
        ...
        [
            ...
            ['Split boundary', f'${abs(INS["top_cover_limit"]):,.0f} per mortgage ($2M home)', 'P20 of conditional deficit — varies by mortgage config'],
            ...
            ['Loaded premium', f'${INS["tail_risk"]["loaded_premium"]:,.0f} per mortgage ($2M home)', f'Fair premium ${INS["tail_risk"]["fair_premium_pv"]:,.0f} + 50% loading'],
    ...
    story.append(Paragraph('8. Quantitative Analysis — v14c Model Results', styles['SectionHead']))
    story.append(Paragraph(
        '<i>All values below are per-mortgage for a representative $2,000,000 property at 80% LVR.</i>',
        styles['BodyText2']
    ))
    ...
    story.append(Paragraph(
        ...
        f'to establish the per-mortgage split boundary (${abs(INS["top_cover_limit"]):,.0f} for a $2M home).',
        styles['FPBullet']
    ))
    story.append(Paragraph(
    ...
    story.append(Paragraph(
        ...
        '~30 years of investment float on the upfront premium, zero prepayment risk, transparent '
        'MLE-calibrated pricing via Monte Carlo simulation, and natural portfolio diversification across '
        'vintages and geographies — all of which are <i>scenario-robust</i>. Expected claim frequency '
        f'ranges from {INS["tail_risk"]["poc"]:.2f}% (base case) to ~3.4% (realistic-central) to ~8.6% '
        '(adverse-plausible) per mortgage. Pricing should anchor to the realistic-central scenario; '
        'attachment, limit and capital should be sized to the adverse-plausible scenario.',
        styles['Callout']
    ))
    ...
    story.append(make_table(
        ...
        [
            ['Cumulative tail-layer claim probability (30yr)',
             f'{INS["tail_risk"]["poc"]:.2f}%', '~3.4%', '2–4% of book', '~1.5%', '~1–2%'],
            ['Loss severity (conditional on deficit)',
             '~21%', '~21%', '15–25%', '40–60%', 'Subordination-dependent'],
            ['Expected loss (30yr, combined)',
             '0.13%', '~0.6–0.8%', '0.6–1.25%', '0.6–0.9%', '0.05–0.15%'],
            ['Claim timing', 'Year 30 only', 'Year 30 only', 'Peaks Year 3–7', 'Any year', 'Any year'],
            ['Float on premium', '~30 yrs', '~30 yrs', '~3–7 yrs', 'N/A', 'N/A'],
            ['Correlation to trad. LMI', 'Near zero', 'Near zero', '—', 'Low', 'Moderate'],
            ['Prepayment / lapse risk', 'None', 'None', 'Significant', 'Moderate', 'Significant'],
        ],
        col_widths=[38*mm, 20*mm, 26*mm, 26*mm, 22*mm, 22*mm]
    ))

    story.append(Spacer(1, 3*mm))
    reins_landscape = [
        '<b>Scenario-sensitive vs scenario-robust metrics.</b> Expected loss and claim probability move '
        'materially across the three parameter scenarios (base → realistic-central roughly quadruples '
        'expected loss). Loss severity (~21%), claim timing (Year 30 only), float duration (~30 years), '
        'and the absence of prepayment risk are <i>structural</i> — they do not move with the parameter '
        'stresses. Reinsurer underwriting should separate these two categories.',
        ...
        '<b>Expected loss band, not single point.</b> The base-case combined expected loss of 0.13% would '
        'place the risk in AA–AAA territory. Under realistic-central parameters the expected loss is closer '
        'to 0.6–0.8%, roughly BBB-BB territory. Under adverse-plausible parameters it is higher still. '
        'Reinsurers should price to the realistic-central expected loss and hold capital against the '
        'adverse-plausible scenario.',
    ]
    ...
    # ---- 11. APPENDIX A: MODEL PARAMETERS ----
    story.append(Paragraph('11. Appendix A: v14c Model Parameters (per-mortgage, $2M home)', styles['SectionHead']))
    ...
    story.append(make_table(
        ...
        [
            ...
            ['Buffer cap', f'{V14["buffer_cap"]*100:.0f}%', 'Maximum annual return credited'],
            ['Buffer floor', f'{V14["buffer_floor"]*100:.0f}%', 'Minimum annual return credited'],
    ...
    story.append(Spacer(1, 5*mm))
    story.append(Paragraph(
        'Source: FutureProofCalculator_Pavel_v14c (003).xlsm | monte_carlo_v14c_003_results.json',
        styles['Footer']
    ))
    story.append(PageBreak())
