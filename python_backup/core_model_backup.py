...
def single_mortgage(total_loan, reinvest_fraction, loan_duration, annual_income, annuity_duration,
  ...
  avg_cash_rate = geometric_mean(cash_rate_series) if type(cash_rate_series) is list else cash_rate_series
  ...
  #print("init reinvest", initial_reinvestment)
  #if initial_reinvestment < 0:
  #  raise ValueError('negative reinvestment account {0:.2f}'.format(initial_reinvestment))
  ...
  for (pathn,S) in price_paths:
    ...
    cummlative_units_sold = 0
    init_units_to_principal = 0
    # includes Y1 income
    loan_size = total_loan * reinvest_fraction + annual_income/4
    if piProgressiveRepayment:
      init_units_to_principal = (annual_income/4) / S0
      loan_size -= annual_income/4 
    ...
    data.append([pathn, 0, year_offset, 0, S0, 0, round(total_loan * reinvest_fraction), holdings, round(holdings*S0),
                round(0), round(max(loan_size - holdings*S0 ,0)), round(holdings*S0 - loan_size - deferred), in_holiday, 0,
                  annual_income/4, 0, False, 0, 0, 0, 0, 0,0,0,0,0,init_units_to_principal,0,0 ])

    last_yearly_hedge_price = S0
    last_5yearly_hedge_price = S0

    for t in range(1, 4*loan_duration+1):

      # We are simulating at the END of the quarter

      s = S[int(t/dt/4)-1] # current price

      #interest on deferred compunds
      #deferred += deferred * interest_on_deferred/4

      # interest
      cash_rate = cash_rate_series[int(t/dt/4)-1] if type(cash_rate_series) is list else cash_rate_series
      loan_interest_rate = cash_rate+wholesale_lending_margin+additional_loan_margins
      interest_due = loan_size * loan_interest_rate/4
      interest_paid = 0
      interest_paid_to_funder=0
      deferred_delta = 0
      units_sold_now =0
      units_to_pool = 0
      units_to_principal = 0
      if in_holiday:
        if holdings*s > holiday_exit:
          in_holiday = False
          if enable_pool and holdings <= insured_units:
            units_to_pool -= interest_due / s
            
          else: 
            holdings -= interest_due / s
            units_sold_now += interest_due / s          
          interest_paid = interest_due
          interest_paid_to_funder = loan_size * (wholesale_lending_margin+cash_rate)/4
          holiday_quarters =0
        else:
          if enable_pool and holdings <= insured_units:
            units_to_pool -= interest_due / s
            ...
            holiday_quarters+=1
            #interest_due = loan_size * (loan_interest_rate-0.0025)/4
            deferred += interest_due
            deferred_delta += interest_due
      else:
        if holdings*s < holiday_enter:
          if enable_pool and holdings <= insured_units:
            units_to_pool -= interest_due / s            
          else: 
            #interest_due = loan_size * (loan_interest_rate-0.0025)/4
          ...
          interest_paid = interest_due
          interest_paid_to_funder = loan_size * (wholesale_lending_margin+cash_rate)/4
          if holdings*s > holiday_exit*superpay_start_factor and deferred>0 and holdings > insured_units:
            surplus_pay = min(max_superpay_factor*interest_due, deferred)
            holdings -= surplus_pay / s
            deferred -= surplus_pay
            deferred_delta -= surplus_pay

            units_sold_now += surplus_pay / s
            interest_paid += surplus_pay
            interest_paid_to_funder += surplus_pay * (wholesale_lending_margin+cash_rate)/loan_interest_rate
      if enable_pool and not in_holiday and deferred<1 and expected_reinvestment is not None and holdings*s > expected_reinvestment[t] and holdings > insured_units:
        excess_units = (holdings*s - expected_reinvestment[t]) / s
      ...
      if hedged:
        if t % 4 ==0:
          ...
          if year_move < -hedging_max_loss:
            #buy_units = (-year_move - hedging_max_loss)*holdings
            # notes jun 10
        ...
        if t % (4*5) ==0:
          ...
          if holdings>adj_holds:
            #sell_units = (year_move - hedging_cap*5)*holdings
      ...
      opportunity_cost =0#+= deferred *cash_rate/4
      cummlative_units_sold += units_sold_now
      cum_interest_paid += interest_paid
      early_termination_deficit = max(deferred , 0)
      #early_termination_deficit = max(loan_size+ deferred - holdings*s - repayment_amount-at_risk_capital, 0)
      # increment loan_size from annual income
      yearly_annuity_income = 0
      # beginning of next year

      if t<annuity_duration*4:
        yearly_annuity_income += annual_income/4

      if t<annuity_duration*4:
        if piProgressiveRepayment:
          units_to_principal = (annual_income/4) / s

      subperform = holiday_quarters >= subperform_loan_threshold_quarters
      year = math.floor((t-1)/4) +1
      quarter = t - 4*math.floor((t-1)/4)
      data.append([pathn, t, year_offset+year, quarter, s, round(interest_due), round(loan_size), holdings, round(holdings*s),
                  deferred, round(max(loan_size - holdings*s ,0)), round(holdings*s - loan_size - deferred+cum_units_to_pool*s), in_holiday, funder_earned,
                   yearly_annuity_income, holiday_quarters, subperform, interest_paid, interest_paid_to_funder, loan_interest_rate, units_sold_now, cummlative_units_sold, 
                   deferred_delta, units_to_pool, cum_units_to_pool,cum_interest_paid, units_to_principal, units_sold_now+units_to_principal, hedge_units_delta ])
      if t<annuity_duration*4:
        if piProgressiveRepayment:          
  ...
  return pd.DataFrame(data, columns=["Path", "Period","Year", "Quarter", "SP500", "Interest", "Loan size",
...
def accounts_table(df):
  #adf =df.copy(deep=True)#pd.DataFrame([df['Quarter']], columns=["Quarter"])
  #adf.drop(['TailNum', 'OriginStateFips', 
  ...
  #adf['Period'] = adf['Quarter'] + 4*adf['Year']
  return adf
